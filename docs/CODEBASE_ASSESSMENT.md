...
| 보안 | 3.2 | 기상청 해당 API가 `http`만 제공되어 전송구간 보완 통제가 필요함 |
...
- 기상청 API가 `http-only`임을 전제로 허용 도메인/경로 검증 강화
- Webhook은 `https` 강제, API는 허용 목록 기반(`http`) 검증
- `http-only` 전송 구간에 대한 운영 보완 통제(네트워크 경계, 모니터링) 명시
...
1. 보안 하드닝(http-only API 보완 통제 + URL 검증)
2. 헬스 저장소 추상화(결합 제거)
