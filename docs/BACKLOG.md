...
| 아키텍처 규율 (추가) | 3.9 | 계층 의존성 경계 위반을 강제 차단하는 자동 검사 규칙이 아직 없음 |
| 릴리스 게이트 단일화 (추가) | 4.0 | 검증 항목은 충분하지만 `make gate` 단일 실행 경로는 미구현 상태 |
...
| RB-604 | P1 | 예정 | 릴리스 게이트 단일화 (추가) | `make gate`(lint+type+docs-check+tests) 단일 명령 추가 | 로컬/CI에서 릴리스 전 판단을 1회 명령으로 재현 가능 |
| RB-605 | P3 | 예정 | 배포 신뢰성/감사 가능성 | PR 템플릿 체크 항목을 CI에서 부분 강제(예: 이벤트 변경 시 문서 동기화 확인) | 템플릿 누락과 실제 검증 결과 간 괴리 감소 |
| RB-701 | P1 | 예정 | 아키텍처 규율 (추가) | 계층 의존성 규칙(`domain` 역참조 금지 등) 자동 검사 도입 | CI에서 의존성 위반 시 실패, 예외 규칙 문서화 |
