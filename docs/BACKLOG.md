...
### 2-1) 이벤트 식별자 충돌 방지 설계 보강

```text
상태: 진행중
중요도: 높음
항목: event_id 충돌 방지 및 저장소 upsert 안정성 강화
근거: event_id가 area/특보 식별 요소를 충분히 포함하지 않아 다중 지역/동시 특보에서
      동일 키 충돌 시 기존 알림이 덮어써질 수 있음.
      (app/domain/models.py, app/repositories/json_state_repo.py, app/repositories/sqlite_state_repo.py)
완료 기준:
- event_id 생성 규칙을 지역/특보 확장 시 충돌 가능성이 낮은 형태로 재정의
- JSON/SQLite 저장소 upsert 및 mark_sent 경로가 신규 규칙에서 동일 동작함을 보장
- 충돌 재현 테스트(동일 시각/서로 다른 지역·특보) 추가
```

### 2-2) 신규 특보 코드 유입 감지력 강화

```text
상태: 진행중
중요도: 중간
항목: 미매핑 특보/명령 코드 처리 정책 개선(N/A 침묵 처리 제거)
근거: warnVar/warnStress/command/cancel 미매핑 값이 N/A로 흡수되어
      신규 코드 유입을 운영에서 조기 탐지하기 어려움.
      (app/domain/code_maps.py, app/services/weather_api.py)
완료 기준:
- 미매핑 코드 발생 시 구조화 경고 이벤트 또는 카운터를 남기도록 처리
- 메시지 생성 시 원본 코드 추적 정보가 유지되도록 도메인 모델/빌더 보강
- 신규/미매핑 코드 테스트 케이스 추가
```

### 2-3) API 요청 파라미터 모델링/확장성 개선
...
### 2-4) 신규 지역 추가 경로 단순화(지역명 매핑 의존 완화)
