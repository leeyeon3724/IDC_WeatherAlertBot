...
| RB-907 | P1 | 진행중 | 상태 저장소 기본전략 정비 | 운영 문서상 상태 무결성/마이그레이션 절차가 중요하며, JSON 기본 운용은 데이터 증가 시 성능/복구 부담 | 기본 저장소 전략을 SQLite 중심으로 정비하고 JSON은 호환/복구 경로로 역할 분리 | 신규 배포 기본값이 SQLite로 동작 + `migrate-state`/`verify-state --strict` CI 스모크 유지 |
